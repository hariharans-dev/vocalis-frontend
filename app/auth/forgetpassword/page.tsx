...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    var response = await fetch("/api/auth/forgetpassword", {
      method: "POST",
      headers: {
    ...
    if ("status" in response && String(response.status) == "success") {
    ...
    <div className="flex h-screen w-screen items-center justify-center bg-background text-foreground">
      <Card className="w-full max-w-md border border-border bg-card shadow-lg p-6 rounded-lg">
        <CardHeader className="text-center space-y-2">
          <h3 className="text-2xl font-semibold">Forget Password for {role}</h3>
